...
def fetch_yf_data(perf_symbols: List[str]) -> pd.DataFrame:
    """
    Fetches fundamental metrics for a list of ticker symbols with rate limit handling.
    """
    if not perf_symbols:
        return pd.DataFrame()
    ...
    for symb in perf_symbols:
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Polite pause between successful requests to prevent getting flagged
                time.sleep(0.5) 
                ...
                if "Too Many Requests" in error_msg or "429" in error_msg:
                    if attempt < max_retries - 1:
                        logging.warning(f"Rate limited on {symb}. Retrying in {retry_delay} seconds...")
                        time.sleep(retry_delay)
                        retry_delay *= 2  # Exponential backoff (5s, 10s, 20s)
                    else:
                        logging.error(f"Max retries reached for {symb}. Skipping fundamental data.")
                else:
                    logging.warning(f"Could not fetch info for {symb}: {e}")
                    break  # Break if it's a completely different error (e.g., delisted ticker)
